...
PostgreSQL uses a connection pool capped at five connections per app process (`DB_POOL_MAX=5`). The default `DB_POOL_MIN=0` lets all idle connections close after `DB_POOL_MAX_IDLE=60` seconds so the managed database can auto-suspend. Leave these defaults in place on the free tier; `DB_POOL_TIMEOUT=15` and `DB_CONNECT_TIMEOUT=10` bound connection waits. Optional pool overrides are `DB_POOL_MIN`, `DB_POOL_MAX`, `DB_POOL_TIMEOUT`, `DB_POOL_MAX_IDLE`, `DB_POOL_MAX_LIFETIME`, `DB_RECONNECT_TIMEOUT`, and `DB_CONNECT_TIMEOUT`.
PandaStack logs include timestamps, model-call start and completion, retries, HTTP status, and safe upstream error identifiers. Add/Search request validation failures also log the client address, path, rejected field names, and validation error types alongside the HTTP 422 access log. Search for `request validation failed` to identify the rejected field. Request values, query text, API keys, and upstream error bodies are not written to these logs.
