...
每个条目包含带有会话窗口头的源消息。单条消息超过证据预算时，返回内容会以 `...` 截断，数据库中的原文仍完整保留。系统不会总结或生成答案。

在 `competition` 模式下，较长的 Add 消息只会在模型调用时分片，数据库仍保存完整原文。较大的 Add 请求可能需要多次调用上游模型。
...
4. **倒数排序融合**：词法、向量候选以及符合条件的日期候选按 `1 / (60 + rank)` 评分合并，并过滤相关性较弱的纯向量候选。

5. **窗口扩展与排序**：匹配候选扩展为包含同一会话相邻消息的窗口。规则排序结合词覆盖、选项匹配、短语匹配和时间意图。没有时间意图、也未检出当天时间戳证据的查询，会使用已配置的重排序模型决定最终证据顺序；调用 `qwen3.7-text-rerank` 时，从规则排序结果中最多选取 500 条候选。模型暂时不可用时使用规则排序，并在下次搜索时重试。询问最早、最新或检出当天时间戳证据的查询保留时间相关的规则排序。

查询明确询问某一天发生的事件时，Search 还会通过现有的按用户和时间建立的索引，查找 `timestamp` 落在该 UTC 日期内的消息。日期仅出现在项目名、工单名、到期日或截止日期中时，不触发这项检索。如果当天没有带时间戳的消息，搜索仍可使用常规重排序。较新的更正可抑制对应的旧单点事实；如果旧消息还包含与查询相关的独立事实，该消息仍可返回。明确要求总结整段历史时，证据选择会保留首位，并在相关性接近的候选中选择不同会话的消息，输出仍按相关性排序。Search 只返回源证据，最终答案由调用方生成。
