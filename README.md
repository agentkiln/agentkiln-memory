...
AgentKiln Memory is an evidence-only memory service for the Textual Memory track of the 2026 Agent Memory Challenge. It implements the public synchronous `Add` and `Search` contract and returns source evidence rather than generated final answers.
...
Use a long random `AML_API_KEY`; configure the same value as the Memory System Key in the evaluation application. Formal mode uses `gpt-4o-mini` for Add/Search model calls and `text-embedding-v4` for embeddings. Search never generates the final answer.
...
- Do not commit `.env`, API keys, Leaderboard Keys, or Memory System Keys.
